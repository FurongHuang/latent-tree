...
//	std::cout << "begin of sparse adjcency matrix" << endl;
	...
	for (int i = 0; i < adj.size(); i++){
		int tmpindex = nodeset[i]->readindex() + 1;// << "\t" <<;
		for (int j = 0; j < adj[i].size(); j++){
			f2 << tmpindex << "\t" << adj[i][j]->readindex() + 1 << "\t" << edgeD[i][j] << endl;
				...
				if (adj[i][j]->readc() != '0'){
					...
					for (long k = 0; k < mat.outerSize(); ++k)
						for (SparseMatrix<double>::InnerIterator it(mat, k); it; ++it){
							f4 << it.row() + 1 << "\t" << it.col() + 1 << "\t" << it.value() << endl;
						}
				}
...
//		cout << "iterator_a: " << iterator_a << endl;
//		assert(distance(adj[it_b].begin(), iterator_a));
		cout << "distance(adj[it_b].begin(), iterator_a): " << distance(adj[it_b].begin(), iterator_a)  << endl;
