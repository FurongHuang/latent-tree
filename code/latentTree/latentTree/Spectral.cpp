...
//	cout << "M rows: " << M.rows() << "M cols: " << M.cols() << endl;
	SparseMatrix<double> random_mat = (M.rows() > 20 * KHID) ? random_embedding_mat((long)M.cols(), k_prime) : random_embedding_mat_dense((long)M.cols(), k_prime);
